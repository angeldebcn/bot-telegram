...
from config_db import get_config, reset_to_defaults, update_config
from stats import get_recent_logs, get_stats, get_top_posters
from menu import render_main_menu_text
...
from helpers import format_duration, format_minutes, safe_username
...
@router.callback_query(F.data.startswith("dnotice:"))
async def cb_delete_notice(cb: CallbackQuery, bot: Bot) -> None:
    chat_id = int(cb.data.split(":")[1])
    if not await _check_admin_or_deny(cb, bot, chat_id):
        return
    cfg = await get_config(chat_id)
    text = (
        "🗑️ <b>Duración del aviso de /delete</b>\n\n"
        "Cuando una admin usa <code>/delete motivo</code> respondiendo a un post, "
        "el bot publica un aviso etiquetando a la chica con el motivo.\n\n"
        "Aquí eliges cuánto tiempo permanece ese aviso antes de autodestruirse.\n\n"
        "<i>♾️ Permanente = el aviso se queda fijo en el grupo.</i>"
    )
    await _safe_edit(
        cb, text, delete_notice_menu(chat_id, int(cfg.get("delete_notice_seconds", 60))),
    )
    await cb.answer()


@router.callback_query(F.data.startswith("dnoticeset:"))
async def cb_delete_notice_set(cb: CallbackQuery, bot: Bot) -> None:
    parts = cb.data.split(":")
    chat_id, value = int(parts[1]), int(parts[2])
    if not await _check_admin_or_deny(cb, bot, chat_id):
        return
    await update_config(chat_id, "delete_notice_seconds", value)
    if value == 0:
        label = "Permanente"
    elif value < 60:
        label = f"{value}s"
...
async def cb_countable_menu(cb: CallbackQuery, bot: Bot) -> None:
    ...
    from builders import countable_menu
